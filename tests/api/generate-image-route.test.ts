...
describe("/api/generate-image", () => {
  ...
  it("generates an image without requiring login", async () => {
    const fetchMock = vi.fn(async () =>
      Response.json({
        data: [{ url: "https://example.com/public-generated.png" }],
      }),
    )
    ...
    expect(body).toMatchObject({
      imageUrl: "https://example.com/public-generated.png",
      model: "doubao-seedream-4-5-251128",
      provider: "Doubao-Seedream-4.5",
    })
    expect(body).not.toHaveProperty("generationId")
  })
  ...
  it("returns a clear key error when Ark rejects the API key", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        Response.json(
